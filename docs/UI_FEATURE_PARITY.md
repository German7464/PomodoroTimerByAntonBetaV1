...
| 11 языков, plural-формы и арабский RTL | `TranslationManager`, `.ts`/`.qm`, выбор в «Оформление» | `test_localization`, проверка каталогов и снимки | Нативный title bar оформляет Windows |
...
Перед релизом на реальном рабочем столе следует проверить 11 языков на основных экранах, переход Arabic → Russian, шесть сочетаний встроенных тем/режимов, пользовательскую тему, паузу и три превышения; затем семь видов виджета при Windows DPI 100%, 125%, 150% и 175%, несколько мониторов, alpha 5% и временную alpha 100%. Автоматические offscreen-снимки подтверждают разметку и отсутствие падения, но не заменяют проверку системных заголовков, ClearType, направления трея и разных видеодрайверов.
